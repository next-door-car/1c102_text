...
uint8_t FLASH_WitePage(uint32_t addr, uint8_t *data, uint32_t num);	//FLASH页编??
uint8_t FLASH_WaitForPeEnd(uint32_t Timeout);					//FLASH等待编程结束
uint8_t FLASH_GetStatus(void);										//FLASH当前状态获??

uint32_t FLASH_ReadPage(uint32_t addr, uint8_t *data, uint32_t num);			//FLASH页读??

FlagStatus FLASH_GetFlagStatus(uint32_t FLASH_FLAG);				//FLASH状态查??
#endif
